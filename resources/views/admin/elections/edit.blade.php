...
@section('title', 'Edit Election - UniVote Admin')
@section('page_heading', 'Edit Election Details')

@section('content')
<div class="max-w-3xl mx-auto bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
    <div class="mb-6 flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
            <h2 class="text-xl font-bold font-heading text-slate-900">Edit: {{ $election->title }}</h2>
            <p class="text-xs text-slate-500">Update schedule dates or toggle active voting session.</p>
        </div>
    ...
    <form action="{{ route('admin.elections.update', $election->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" required value="{{ old('title', $election->title) }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">
        </div>

        <div>
            <label for="description" class="block text-xs font-bold text-slate-700 mb-1">Description / Purpose</label>
            <textarea name="description" id="description" rows="3" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">{{ old('description', $election->description) }}</textarea>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                <label for="start_date" class="block text-xs font-bold text-slate-700 mb-1">Start Date & Time *</label>
                <input type="datetime-local" name="start_date" id="start_date" required value="{{ old('start_date', $election->start_date ? $election->start_date->format('Y-m-d\TH:i') : '') }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">
            </div>

            <div>
                <label for="end_date" class="block text-xs font-bold text-slate-700 mb-1">End Date & Time *</label>
                <input type="datetime-local" name="end_date" id="end_date" required value="{{ old('end_date', $election->end_date ? $election->end_date->format('Y-m-d\TH:i') : '') }}" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">
            </div>
        </div>
        ...
        <div>
            ...
            <select name="status" id="status" required class="w-full sm:w-1/2 px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">
                ...
                <option value="active" {{ old('status', $election->status) === 'active' ? 'selected' : '' }}>Active (Set as Current Election)</option>
        ...
        <div class="flex items-center justify-end space-x-4 pt-6 border-t border-slate-100">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white font-bold px-6 py-2.5 rounded-xl text-xs shadow-md shadow-blue-600/30">
                <i class="fa-solid fa-save mr-1.5"></i> Update Election
            </button>
        </div>
    </form>
</div>
