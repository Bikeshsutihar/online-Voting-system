...
@section('title', 'Add New Candidate - UniVote Admin')
@section('page_heading', 'Add New Candidate')

@section('content')
<div class="max-w-4xl mx-auto bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
    <div class="mb-6 flex items-center justify-between border-b border-slate-100 pb-4">
        <div>
            <h2 class="text-xl font-bold font-heading text-slate-900">Candidate Information</h2>
            <p class="text-xs text-slate-500">Fill in student details and upload party logo & student ID card photo.</p>
        </div>
    ...
    <form action="{{ route('admin.candidates.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" id="name" required value="{{ old('name') }}" placeholder="e.g. Alex Johnson" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">
            </div>
            ...
            <div>
                ...
                <input type="text" name="student_id" id="student_id" required value="{{ old('student_id') }}" placeholder="STU-2026-001" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">
            </div>

            <div>
                <label for="phone_no" class="block text-xs font-bold text-slate-700 mb-1">Phone Number *</label>
                <input type="text" name="phone_no" id="phone_no" required value="{{ old('phone_no') }}" placeholder="+1 555-0192" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">
            </div>
            ...
            <div>
                <label for="party_name" class="block text-xs font-bold text-slate-700 mb-1">Party / Student Alliance Name *</label>
                <input type="text" name="party_name" id="party_name" required value="{{ old('party_name') }}" placeholder="Progressive Student Alliance" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-xl text-xs focus:outline-none focus:border-blue-500">
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
            <div>
                ...
                <p class="text-[11px] text-slate-400 mt-1">Saved directly to public/uploads/candidate_logos</p>
            </div>
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-400 mt-1">Saved directly to public/uploads/candidate_ids</p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end space-x-4 pt-6 border-t border-slate-100">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white font-bold px-6 py-2.5 rounded-xl text-xs shadow-md shadow-blue-600/30">
                <i class="fa-solid fa-save mr-1.5"></i> Create Candidate
            </button>
        </div>
    </form>
</div>
